...
const ReceiveTransfer = () => {
  ...
  useEffect(() => {
    ...
    if (rate && formData.amount) {
      setCurrentRate(rate.buy_rate);
      setConvertedAmount(parseFloat(formData.amount) * rate.buy_rate);
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.amount || !formData.senderName || !formData.senderPhone) {
      toast({
        title: "خطا",
    ...
    if (error) {
      toast({
        title: "خطا",
        description: "مشکلی در ثبت درخواست رخ داد",
        variant: "destructive",
      });
    } else {
      toast({
        title: "موفق",
        description: "درخواست دریافت حواله ثبت شد",
      });
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <Card className="border-border/50 bg-card/80 backdrop-blur-xl">
            <CardHeader className="text-center">
              ...
              <CardTitle className="text-2xl">دریافت حواله</CardTitle>
              <CardDescription>ثبت درخواست دریافت حواله از فرستنده</CardDescription>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  {convertedAmount > 0 && (
                    <p className="text-sm text-muted-foreground">
                      مبلغ دریافتی: <span className="text-primary font-mono">{convertedAmount.toFixed(2)} {formData.toCurrency}</span>
                      {" | "}
                      نرخ: <span className="font-mono">{currentRate}</span>
                    </p>
                  )}
                </div>

                <div className="border-t border-border/50 pt-6">
                  <h3 className="font-semibold mb-4">اطلاعات فرستنده</h3>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      <Label>شماره تماس فرستنده *</Label>
                      <Input
                ...
                <div className="bg-secondary/30 rounded-xl p-4">
                  ...
                  <div className="flex justify-between font-semibold pt-2 border-t border-border/50">
                    ...
                    <span className="text-primary font-mono">{convertedAmount.toFixed(2)} {formData.toCurrency}</span>
                  </div>
                </div>

                <Button type="submit" className="w-full" size="lg" disabled={isLoading}>
                  {isLoading ? <Loader2 className="h-4 w-4 animate-spin ml-2" /> : <Download className="h-4 w-4 ml-2" />}
                  ثبت درخواست
                </Button>
              </form>
            </CardContent>
